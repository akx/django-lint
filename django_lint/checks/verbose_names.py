...
from django_lint.ast_utils import (
    find_all_model_definitions, find_all_model_fields,
    get_call_kwargs, get_model_meta_assignments,
)
...
class BaseVerboseNameCapitalizationCheck(Check):
    def _check_verbose_name(self, context_name, vn_node):
        if not (vn_node and isinstance(vn_node, Const) and isinstance(vn_node.value, str)):
            return
...
class FieldVerboseNameCapitalization(BaseVerboseNameCapitalizationCheck):
    ...
    def check(self):
        for cdef, assign in find_all_model_fields(self.context, self.file_context.ast):
            ...
            yield from self._check_verbose_name(context_name=assign.targets[0], vn_node=vn_node)
            ...
            for field in ('verbose_name', 'verbose_name_plural'):
                if field in assignments:
                    ...
                    if vn_node and isinstance(vn_node, Const) and isinstance(vn_node.value, str):
                        yield from self._check_verbose_name(context_name=cdef.name, vn_node=vn_node)
